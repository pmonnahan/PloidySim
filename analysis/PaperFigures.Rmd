...
pardir = "/Users/pmonnahan/bkup"
ms = read.table(paste(pardir, "/Documents/Research/PloidySim/data/mssel_out_8-27-19.txt", sep = ""), head = T)
ms %<>% mutate(ints = as.numeric(as.character(cut(bp.start, breaks = 50, labels = seq(1,50))))) # label (bin) windows into larger intervals

ms2 = ms %<>% filter(DS != "-9")
ms %<>% filter(DS == "-9")

ds.ehh.files = c(paste(pardir,"/Documents/Research/PloidySim/rehh/rehh_DS.csv", sep = ""))
nds.ehh.files = c(paste(pardir,"/Documents/Research/PloidySim/rehh/rehh_tp2_do0.5_nDS.csv", sep = ""))
traj=read.table("/Users/pmonnahan/bkup//Documents/Research/PloidySim/data/Ploidy_Forward_Sim_results.txt",head=T, sep=",")
rehh = read.csv("/Users/pmonnahan/bkup/Documents/Research/PloidySim/rehh/rehh_calcs_do0.5_fG1_nDS.csv")
