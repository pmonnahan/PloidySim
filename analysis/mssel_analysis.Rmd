...
CV <- function(data){(sd(data, na.rm=T)/(mean(data, na.rm=T)))*100}
...
ms6.1 = ms6 %>% mutate("TajD" = med.d, "FayWuH" = med.h, "ZengsE" = med.e, "Fst" = med.fst, "Dxy" = med.dxy, "XPEHH" = med.xp, "Rsb" = med.rsb) %>% select(sim_ploidy, DS, TajD, ZengsE, FayWuH, Fst, Dxy, XPEHH, Rsb) %>% melt(., id.var = c("sim_ploidy", "DS")) %>% mutate(stat = "MaxValue")

ms6.2 = ms6 %>% mutate("TajD" = size.taj, "FayWuH" = size.h, "ZengsE" = size.e, "Fst" = size.fst, "Dxy" = size.dxy, "XPEHH" = size.xp, "Rsb" = size.rsb) %>% select(sim_ploidy, DS, TajD, ZengsE, FayWuH, Fst, Dxy, XPEHH, Rsb) %>% melt(., id.var = c("sim_ploidy", "DS")) %>% mutate(stat = "AUC")
...
MS6.1 = ms6 %>% mutate("TajD" = med.d, "FayWuH" = med.h, "ZengsE" = med.e, "Fst" = med.fst, "Dxy" = med.dxy, "XPEHH" = med.xp, "Rsb" = med.rsb) %>% select(sim_ploidy, DS, TajD, ZengsE, FayWuH, Fst, Dxy, XPEHH, Rsb) %>% group_by(sim_ploidy, DS) %>% summarize_all(c("CV", "lCV", "uCV"))
