...
ggplot(traj2, aes(x=gen/1000, y=freq, color=as.factor(ploidy))) + geom_line() + scale_color_manual(name="Ploidy",values=c('red','blue','green','purple')) + theme_bw() + xlab("Generation (1k)") + ylab("Allele Frequency") + facet_wrap(~dom, scales="free_x", nrow = 2)
```
...
params = expand.grid(s = s, N = N, ploidy = c)

params %<>% mutate(fixprob = pmap(list(s, N, ploidy), getFix)) %>% unnest()


ggplot(params, aes(y = fixprob, x = N, color = as.factor(ploidy))) + geom_line() + geom_hline(yintercept = 2*s, linetype='dashed') + scale_x_log10() + xlab("Population Size") + ylab("Fixation Probability") + scale_color_discrete(name="Ploidy")
```
