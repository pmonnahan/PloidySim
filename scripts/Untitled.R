...
co=readMS(file="~/Documents/Research/PloidySim/testing/test_out.txt")
pos=read.ms.output(file.ms.output = "~/Documents/Research/PloidySim/testing/test_traj.txt")
pos2=posb$positions
cow=sliding.window.transform(co,width=20,jump=10,type=1,whole.data = F)
cow=diversity.stats(cow)
cow=linkage.stats(cow)
cow=F_ST.stats(cow)
df1 = makeOnePop.df(cow,500000,pos2,1)

ggplot(df2,aes(x=mid,y=Pi,color=rep)) + geom_line()
ggplot(df1,aes(x=mid,y=Pi,color=rep)) + geom_line()
